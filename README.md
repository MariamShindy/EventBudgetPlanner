...
```bash
git clone https://github.com/yourusername/EventBudgetPlanner.git
...
- Keep your email app passwords secure
...
- Ensure SQL Server LocalDB is installed (Windows)
- Check the connection string in `appsettings.json`
- Verify SQL Server service is running

**Migration Errors:**
- Delete the database and let it recreate on next run
- Check Entity Framework migrations are up to date

**Port Already in Use:**
- Change the port in `launchSettings.json` or `appsettings.json`
- Update the frontend `environment.ts` to match
...
- Ensure the API URL in `environment.ts` matches the backend URL
...
Once the backend is running, visit `https://localhost:7199/swagger` to view the interactive API documentation.
...
**Note**: This README assumes you're setting up the project for the first time. If you encounter any issues, please check the troubleshooting section or open an issue on GitHub.
